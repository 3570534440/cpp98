...
class TeachingStudent : public Student, public Teacher//助教继承两个子类
{
public:
...
Person::Person(std::string theName)//基类的构造器
{
    name = theName;
}
...
Teacher::Teacher(std::string theName, std::string theClass) : Person(theName)//继承基类的构造器
{
    classes = theClass;
}
...
Student::Student(std::string theName, std::string theClass) : Person(theName)//继承基类的构造器
{
    classes = theClass;
}
...
TeachingStudent::TeachingStudent(std::string theName,
                                 ...
                                 : Teacher(theName, classTeaching), Student(theName, classAttending)//继承两个子类，中间用逗号，括号内的不是数据类型
{
}
